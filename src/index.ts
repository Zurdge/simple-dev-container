...
server.registerTool(
  "task_start",
  {
    ...
    inputSchema: {
      repo: z.string().describe("Repo key"),
...
server.registerTool(
  "repo_list",
  { description: "List repos available that can be used with task_start." },
  async () => {
...
server.registerTool(
  ...
  async ({ task_name }) => {
    ...
    if (stop.exitCode !== 0)
      return err(`Failed to stop: ${stop.stderr.trim()}`);
    const rm = run(["docker", "rm", task_name]);
    if (rm.exitCode !== 0)
      return err(`Stopped but failed to remove: ${rm.stderr.trim()}`);
...
server.registerTool(
  ...
  async ({ task_name, command }) => {
    const result = run(
      ["docker", "exec", task_name, "sh", "-c", command],
      120_000,
    );
    const output = (result.stdout + result.stderr).trim();
    const status = result.exitCode !== 0 ? `[exit ${result.exitCode}]` : "[ok]";
